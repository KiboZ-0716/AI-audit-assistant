...
import { AuditProgressStepper } from '@/components/business/AuditProgressStepper'
...
import { AUDIT_PLANT_OPTIONS, isPlantDigitized } from '@/mocks/seed/plants'
...
export function AuditTaskPage() {
  ...
  if (!projectId || !workspace || !project) return null
  ...
  if (isAiRunning) {
    return (
      <div className="grid gap-4 lg:grid-cols-[300px_1fr]">
        <AuditProgressStepper ai={workspace.ai} />
  ...
  return (
    <div className="space-y-5">
      <section className="border border-slate-200 bg-white px-5 py-4">
        <h2 className="text-sm font-semibold text-slate-900">任务配置</h2>
        <p className="mt-1 text-xs text-slate-500">选择审核周期与基地，配置子要素数据源后启动 AI 审核</p>
        <div className="mt-4 flex flex-wrap items-end gap-4">
          ...
          <label className="block text-xs text-slate-500">
            ...
            <select
              ...
              onChange={(e) => updateWorkspaceBasics(projectId, { plant: e.target.value })}
            ...
            <SubElementConfigPanel
              ...
              onToggleSource={(key) => {
                ...
                if (
                  key === 'systemEnabled' &&
                  enabled &&
                  selectedNode.dataSources.systemFields.length === 0
                ) {
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border border-slate-200 bg-white px-5 py-4">
              <div className="text-sm text-slate-500">
                {startError ? (
                  ...
                  <span>启动后将生成 BIP 问题管理表初稿，并进入 AI 审核流程</span>
                )}
              </div>
              <button
                type="button"
